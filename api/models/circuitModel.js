...
const Circuit = {
   ...
   * @param {boolean} onlyActive - Filtre uniquement les circuits actifs
   */
  getAll: async (onlyActive = true) => {
    try {
      ...
      // Tri par date de début de voyage (du plus proche au plus lointain)
  ...
  getById: async (id) => {
    try {
      const sql = 'SELECT * FROM circuits WHERE id = $1'; 
  ...
  delete: async (id) => {
    try {
      const sql = 'DELETE FROM circuits WHERE id = $1 RETURNING *'; 
      const { rows } = await db.query(sql, [id]); 
      return rows[0]; 
    } catch (error) {
